...
];document.querySelector('#faq-list').innerHTML=faqs.map((f,i)=>`<div class="faq-item ${i===0?'open':''}"><button class="faq-q"><span>${f[0]}</span><img class="faq-icon faq-icon-chevron" src="assets/ui/faq-chevron-down.svg" alt=""><img class="faq-icon faq-icon-close" src="assets/ui/faq-close.svg" alt=""></button><div class="faq-a">${f[1]}<a class="pill pill-green faq-cta" href="#pricing">Take a product tour</a></div></div>`).join('');document.querySelectorAll('.faq-q').forEach(q=>q.addEventListener('click',()=>q.parentElement.classList.toggle('open')));
(function(){
